...
describe('Mars Rovers', () => {
  ...
  it('Changes direction to East when facing North and receiving the "r" command.', () => {
    const grid = rover([0, 0], 'N', [1, 1], ['r'])
    expect(grid[0][0]).toBe("E")
  })
  ...
  it('Changes direction to West when facing South and receiving the "r" command.', () => {
    const grid = rover([0, 0], 'S', [1, 1], ['r'])
    expect(grid[0][0]).toBe("W")
  })

  it('Changes direction to North when facing West and receiving the "r" command.', () => {
    const grid = rover([0, 0], 'W', [1, 1], ['r'])
    expect(grid[0][0]).toBe("N")
  })

  it('Changes direction to West when facing North and receiving the "l" command.', () => {
    const grid = rover([0, 0], 'N', [1, 1], ['l'])
    expect(grid[0][0]).toBe("W")
  })

  it('Changes direction to North when facing East and receiving the "l" command.', () => {
    const grid = rover([0, 0], 'E', [1, 1], ['l'])
    expect(grid[0][0]).toBe("N")
  })

  it('Changes direction to East when facing South and receiving the "l" command.', () => {
    const grid = rover([0, 0], 'S', [1, 1], ['l'])
    expect(grid[0][0]).toBe("E")
  })

  it('Changes direction to South when facing West and receiving the "l" command.', () => {
    const grid = rover([0, 0], 'W', [1, 1], ['l'])
    expect(grid[0][0]).toBe("S")
  })

  it('The direction the rover is facing changes correctly when it turns more than 360 degrees to the left. ', () => {
    const grid = rover([0, 0], 'S', [1, 1], ['l', 'l', 'l', 'l', 'l', 'l'])
    expect(grid[0][0]).toBe("N")
  })

  it('The direction the rover is facing changes correctly when it turns more than 360 degrees to the right. ', () => {
    const grid = rover([0, 0], 'S', [1, 1], ['r', 'r', 'r', 'r', 'r', 'r', 'r', 'r', 'r'])
    expect(grid[0][0]).toBe("W")
  })

  it('When moving to the left, the rover stays in the same position if it encounters the grid\'s limit. ', () => {
    const grid = rover([1, 0], 'W', [2, 2], ['f'])
    expect(grid[1][0]).toBe("W")
  })

  it('When moving to the right, the rover stays in the same position if it encounters the grid\'s limit. ', () => {
    const grid = rover([1, 1], 'E', [2, 2], ['f'])
    expect(grid[1][1]).toBe("E")
  })

  it('When moving upwards, the rover stays in the same position if it encounters the grid\'s limit. ', () => {
    const grid = rover([0, 0], 'N', [2, 2], ['f'])
    expect(grid[0][0]).toBe("N")
  })

  it('When moving downwards, the rover stays in the same position if it encounters the grid\'s limit. ', () => {
    const grid = rover([1, 0], 'S', [2, 2], ['f'])
